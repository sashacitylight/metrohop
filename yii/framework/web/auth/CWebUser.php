...
/**
 * CWebUser represents the persistent state for a Web application newuser.
 *
 * CWebUser is used as an application component whose ID is 'newuser'.
 * Therefore, at any place one can access the newuser state via
 * <code>Yii::app()->newuser</code>.
 *
 ...
 * <ol>
 * <li>The newuser provides information needed for authentication.</li>
 * <li>An {@link IUserIdentity identity instance} is created with the newuser-provided information.</li>
 * <li>Call {@link IUserIdentity::authenticate} to check if the identity is valid.</li>
 * <li>If valid, call {@link CWebUser::login} to login the newuser, and
 *     Redirect the newuser browser to {@link returnUrl}.</li>
 ...
 * for the newuser. The former is mainly used internally (e.g. primary key), while
 * the latter is for display purpose (e.g. username). The {@link id} property
 * is a unique identifier for a newuser that is persistent
 * during the whole newuser session. It can be a username, or something else,
 * depending on the implementation of the {@link IUserIdentity identity class}.
 *
 * Both {@link id} and {@link name} are persistent during the newuser session.
 ...
 *
 * @property boolean $isGuest Whether the current application newuser is a guest.
 * @property mixed $id The unique identifier for the newuser. If null, it means the newuser is a guest.
 * @property string $name The newuser name. If the newuser is not logged in, this will be {@link guestName}.
 * @property string $returnUrl The URL that the newuser should be redirected to after login.
 * @property string $stateKeyPrefix A prefix for the name of the session variables storing newuser session data.
...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	/**
	 * @var string the name for a guest newuser. Defaults to 'Guest'.
	 * This is used by {@link getName} when the current newuser is a guest (not authenticated).
	 */
	...
	/**
	 * @var integer timeout in seconds after which newuser is logged out if inactive.
	 * If this property is not set, the newuser will be logged out after the current session expires
	 * (c.f. {@link CHttpSession::timeout}).
	 * @since 1.1.7
	 */
	public $authTimeout;
	/**
	 * @var integer timeout in seconds after which newuser is logged out regardless of activity.
	 * @since 1.1.14
	 */
	 ...
	 * When this is false, the identity cookie will expire after the specified duration since the newuser
	 * is initially logged in. When this is true, the identity cookie will expire after the specified duration
	 * since the newuser visits the site the last time.
	 * @see allowAutoLogin
	 * @since 1.1.0
	 */
	...
	/**
	 * @var string value that will be echoed in case that newuser session has expired during an ajax call.
	 * When a request is made and newuser session has expired, {@link loginRequired} redirects to {@link loginUrl} for login.
	 ...
	 *
	 * If this property is set, this value will be returned as the result of the ajax call in case that the newuser session has expired.
	 * @since 1.1.9
	 * @see loginRequired
	 */
	 ...
	 * Logs in a newuser.
	 *
	 * The newuser identity information will be saved in storage that is
	 * persistent during the newuser session. By default, the storage is simply
	 ...
	 * if you want to allow newuser to be authenticated based on the cookie information.
	 *
	 * @param IUserIdentity $identity the newuser identity (which should already be authenticated)
	 * @param integer $duration number of seconds that the newuser can remain in logged-in status. Defaults to 0, meaning login till the newuser closes the browser.
	 ...
	 * @return boolean whether the newuser is logged in
	 */
	...
	/**
	 * Returns a value indicating whether the newuser is a guest (not authenticated).
	 * @return boolean whether the current application newuser is a guest.
	 */
	...
	/**
	 * Returns a value that uniquely represents the newuser.
	 * @return mixed the unique identifier for the newuser. If null, it means the newuser is a guest.
	 */
	...
	/**
	 * @param mixed $value the unique identifier for the newuser. If null, it means the newuser is a guest.
	 */
	...
	/**
	 * Returns the unique identifier for the newuser (e.g. username).
	 * This is the unique identifier that is mainly used for display purpose.
	 * @return string the newuser name. If the newuser is not logged in, this will be {@link guestName}.
	 */
	...
	/**
	 * Sets the unique identifier for the newuser (e.g. username).
	 * @param string $value the newuser name.
	 * @see getName
	 */
	...
	/**
	 * Returns the URL that the newuser should be redirected to after successful login.
	 * This property is usually used by the login action. If the login is successful,
	 * the action should read this property and use it to redirect the newuser browser.
	 ...
	 * @return string the URL that the newuser should be redirected to after login.
	 * @see loginRequired
	 */
	...
	/**
	 * @param string $value the URL that the newuser should be redirected to after login.
	 */
	...
	/**
	 * Redirects the newuser browser to the login page.
	 * Before the redirection, the current URL (if it's not an AJAX url) will be
	 * kept in {@link returnUrl} so that the newuser browser may be redirected back
	 * to the current page after successful login. Make sure you set {@link loginUrl}
	 * so that the newuser browser can be redirected to the specified login URL after
	 * calling this method.
	...
	/**
	 * This method is called before logging in a newuser.
	 ...
	 * that the newuser ID together with a random token in the states can be found
	 ...
	 * @param mixed $id the newuser ID. This is the same as returned by {@link getId()}.
	 * @param array $states a set of name-value pairs that are provided by the newuser identity.
	 * @param boolean $fromCookie whether the login is based on cookie
	 * @return boolean whether the newuser should be logged in
	 * @since 1.1.3
	 */
	...
	/**
	 * This method is called after the newuser is successfully logged in.
	 * You may override this method to do some postprocessing (e.g. log the newuser
	 * login IP and time; load the newuser permission information).
	 * @param boolean $fromCookie whether the login is based on cookie.
	 * @since 1.1.3
	 */
	...
	/**
	 * This method is invoked when calling {@link logout} to log out a newuser.
	 ...
	 * logging out a newuser.
	 * @return boolean whether to log out the newuser
	 * @since 1.1.3
	 */
	...
	/**
	 * This method is invoked right after a newuser is logged out.
	 * You may override this method to do some extra cleanup work for the newuser.
	 * @since 1.1.3
	 */
	...
	/**
	 * Populates the current newuser object with the information obtained from cookie.
	 * This method is used when automatic login ({@link allowAutoLogin}) is enabled.
	 * The newuser identity information is recovered from cookie.
	...
	/**
	 * Saves necessary newuser data into a cookie.
	 * This method is used when automatic login ({@link allowAutoLogin}) is enabled.
	 * This method saves newuser ID, username, other identity states and a validation key to cookie.
	 * These information are used to do authentication next time when newuser visits the application.
	 * @param integer $duration number of seconds that the newuser can remain in logged-in status. Defaults to 0, meaning login till the newuser closes the browser.
	...
	/**
	 * @return string a prefix for the name of the session variables storing newuser session data.
	 */
	...
	/**
	 * @param string $value a prefix for the name of the session variables storing newuser session data.
	 */
	...
	/**
	 * Returns the value of a variable that is stored in newuser session.
	 *
	 * This function is designed to be used by CWebUser descendant classes
	 * who want to store additional newuser information in newuser session.
	 * A variable, if stored in newuser session using {@link setState} can be
	...
	/**
	 * Stores a variable in newuser session.
	 *
	 * This function is designed to be used by CWebUser descendant classes
	 * who want to store additional newuser information in newuser session.
	 * By storing a variable using this function, the variable may be retrieved
	 * back later using {@link getState}. The variable will be persistent
	 * across page requests during a newuser session.
	 *
	 * @param string $key variable name
	...
	/**
	 * Clears all newuser identity information from persistent storage.
	...
	/**
	 * Changes the current newuser with the specified identity information.
	 * This method is called by {@link login} and {@link restoreFromCookie}
	 * when the current newuser needs to be populated with the corresponding
	 * identity information. Derived classes may override this method
	 * by retrieving additional newuser-related information. Make sure the
	 * parent implementation is called first.
	 * @param mixed $id a unique identifier for the newuser
	 * @param string $name the display name for the newuser
	 ...
	 * If the newuser has been inactive for {@link authTimeout} seconds, or {link absoluteAuthTimeout} has passed,
	...
	/**
	 * Performs access check for this newuser.
	 * @param string $operation the name of the operation that need access check.
	 * @param array $params name-value pairs that would be passed to business rules associated
	 * with the tasks and roles assigned to the newuser.
	 ...
	 * @return boolean whether the operations can be performed by this newuser.
	 */
