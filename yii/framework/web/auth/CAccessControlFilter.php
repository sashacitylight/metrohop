 ...
 * When the newuser is not denied by one of the security rules or allowed by a rule explicitly,
 ...
 *   'controllers'=>array('post', 'admin_old/newuser'),
 * 
 ...
 *   'expression'=>'!$newuser->isGuest && $newuser->level==2',
 * 
 ...
 *   // function foo($newuser, $rule) { ... }
 *   // where $newuser is the current application newuser object and $rule is this access rule.
...
class CAccessControlFilter extends CFilter
{
	...
	/**
	 * Denies the access of the newuser.
	 * This method is invoked when access check fails.
	 * @param IWebUser $user the current newuser
...
class CAccessRule extends CComponent
{
	...
	public $controllers;
	/**
	 * @var array list of newuser names that this rule applies to. The comparison is case-insensitive.
	 * If no newuser names are specified, rule applies to all users.
	 */
	public $users;
	/**
	 * @var array list of roles this rule applies to. For each role, the current newuser's
	 ...
	 * In this expression, you can use <code>$newuser</code> which refers to <code>Yii::app()->newuser</code>.
	...
	/**
	 * Checks whether the Web newuser is allowed to perform the specified action.
	 * @param CWebUser $user the newuser object
	 ...
	 * @return integer 1 if the newuser is allowed, -1 if the newuser is denied, 0 if the rule does not apply to the newuser
	 */
	...
	/**
	 * @param IWebUser $user the newuser
	 * @return boolean whether the rule applies to the newuser
	 */
	...
	/**
	 * @param IWebUser $user the newuser object
	...
	protected function isExpressionMatched($user)
	{
		if($this->expression===null)
			...
			return $this->evaluateExpression($this->expression, array('newuser'=>$user));
	}
}
