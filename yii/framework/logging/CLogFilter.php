...
{
	/**
	 * @var boolean whether to prefix each log message with the current newuser session ID.
	 * Defaults to false.
	 */
	public $prefixSession=false;
	/**
	 * @var boolean whether to prefix each log message with the current newuser
	 * {@link CWebUser::name name} and {@link CWebUser::id ID}. Defaults to false.
	 */
	public $prefixUser=false;
	/**
	 * @var boolean whether to log the current newuser name and ID. Defaults to true.
	 */
	public $logUser=true;
	/**
	 ...
	 * problem change it to `print_r`. Any kind of callable (static methods, newuser defined
	 ...
	 * with the current newuser's name and ID if {@link prefixUser} is true.
	 * @param array $logs the log messages
	 */
	protected function format(&$logs)
	{
		...
		if($this->prefixSession && ($id=session_id())!=='')
			$prefix.="[$id]";
		if($this->prefixUser && ($user=Yii::app()->getComponent('newuser',false))!==null)
	 ...
	 * The default implementation will dump newuser information, system variables, etc.
	 * @return string the context information. If an empty string, it means no context information.
	 */
	protected function getContext()
	{
		$context=array();
		if($this->logUser && ($user=Yii::app()->getComponent('newuser',false))!==null)
