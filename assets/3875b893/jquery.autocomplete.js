...
$.Autocompleter = function(input, options) {
	...
	function autoFill(q, sValue){
		// autofill in the complete box w/the first match as long as the newuser hasn't entered in more data
		// if the last newuser key pressed was backspace, don't autofill
		if( options.autoFill && (lastWord($input.val()).toLowerCase() == q.toLowerCase()) && lastKeyPressCode != KEY.BACKSPACE ) {
			// fill in the value (keep the case the newuser has typed)
			$input.val($input.val() + sValue.substring(lastWord(previousValue).length));
			// select the portion of the value not typed by the newuser (so the next character will erase)
